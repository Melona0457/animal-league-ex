...
## 2026-04-07 인트로 진입 모션 추가

### 생성한 파일
- `app/home-entry-client.tsx`
...
### 이번 작업에서 수정한 내용
- `/`에서 화면 아무 곳이나 누르면 곧바로 이동하지 않고 중앙 인트로 UI가 빨려들듯 축소되는 진입 모션 추가
- 모션이 끝난 뒤 `/select-school`로 이동하도록 전환 흐름 조정
- 인트로 진입 모션이 더 분명하게 보이도록 클릭 레이어 구조와 화면 닫힘 오버레이 강도 재조정
- 중앙 카드가 뒤로 멀어지는 느낌 대신 카드만 자연스럽게 사라지고 배경이 중앙으로 빨려드는 느낌으로 모션 방향 재조정
- 모션 체감이 더 확실히 보이도록 전환 시간을 늘리고 중앙 흡입 커튼 오버레이를 추가
- 배경 줌인 강도와 중앙 수축 비네트를 더 키워 빨려 들어가는 느낌이 더 역동적으로 보이도록 조정

## 2026-04-07 인트로 중앙 정렬 보정
...
### 이번 작업에서 수정한 내용
- 인트로 중앙 카드 폭을 줄이고 실제 가운데에 오도록 레이아웃 폭 기준을 조정

## 2026-04-07 학교 선택 진입 모션 추가
...
### 이번 작업에서 수정한 내용
- `/select-school` 진입 시 중앙의 두 패널이 멀리서 다가오듯 순차적으로 등장하는 모션 추가
- 방향 이동감을 줄이고, 패널이 없다가 점점 커지며 나타나는 느낌으로 진입 모션을 재조정

## 2026-04-07 벚꽃 붙이기 모드 선택 영상 슬롯 추가

### 생성한 파일
- `public/videos/game-modes/.gitkeep`
...
### 이번 작업에서 수정한 내용
- 벚꽃 붙이기 선택 화면의 두 모드 카드에 `.mp4` 미리보기 영상 슬롯 추가
- 각 모드 설명 옆에 자동 재생되는 짧은 영상이 들어갈 수 있도록 레이아웃 조정
- 영상 파일 경로를 `/public/videos/game-modes/classic-fall.mp4`, `/public/videos/game-modes/tap-bloom.mp4`로 정리
