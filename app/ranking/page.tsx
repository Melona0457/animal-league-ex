...
export default async function RankingPage({ searchParams }: RankingPageProps) {
  ...
  const currentSchoolId = params.schoolId ?? "yonsei";
  const schools = sort === "name" ? getSchoolsByName() : getRankedSchools();
  const podiumSchools = getRankedSchools().slice(0, 3);
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-4">
        <header className="rounded-2xl border border-stone-200 bg-white p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-sm">
            <Link
              href={`/game?schoolId=${currentSchoolId}`}
              className="rounded-xl border border-stone-200 px-3 py-2"
            >
              벚꽃 붙이기
            </Link>
            <Link
              href={`/community?schoolId=${currentSchoolId}`}
              className="rounded-xl border border-stone-200 px-3 py-2"
            >
              커뮤니티
            </Link>
          ...
          <section className="rounded-[2rem] border border-stone-200 bg-white p-4">
            ...
            <div className="mt-5 flex items-end justify-center gap-3 overflow-x-auto pb-2">
              {[podiumSchools[1], podiumSchools[0], podiumSchools[2]].map((school, index) => {
                ...
                const heightClass =
                  index === 1 ? "h-40 bg-rose-400 text-white" : index === 0 ? "h-32 bg-stone-200 text-stone-900" : "h-24 bg-amber-100 text-stone-900";
